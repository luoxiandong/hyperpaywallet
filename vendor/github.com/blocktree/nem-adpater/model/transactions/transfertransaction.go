...
import (
	...
	"github.com/blocktree/nem-adpater/base"
...
func (r *Transfer) Prepare(common Common, network int) (base.Transaction, float64, error) {
	var msc txPrepare
	...
	if r.IsMultisig {
		if r.MultisigAccount != "" {
			if !utils.IsPublicKeyValid(r.MultisigAccount) {
				err := errors.New("It is not PublicKeyValid!")
				return nil, 0, err
			}
	...
	msc.amount = math.Round(r.Amount)
	...
	if r.IsMultisig && r.MultisigAccount != "" {
		return MultisigWrapper(kp.PublicString(), rt, msc.due, network), rt.Fee, nil
	}
	return rt, rt.Fee, nil
}
...
// link http://bob.nem.ninja/docs/#transferTransaction
func (r *Transfer) PrepareMosaic(common Common, mosaicDefinitionMetaDataPair map[string]base.MosaicDefinition, network int) base.Transaction {
	...
	for _, b := range r.Mosaics {
		...
		// }
		supplys[fullMosaicName] = float64(0)
	}
