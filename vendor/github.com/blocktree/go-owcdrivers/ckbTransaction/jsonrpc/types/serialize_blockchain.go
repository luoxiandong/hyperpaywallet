...
// Serialize transaction
func (t *Transaction) SerializeRawTx() ([]byte, error) {
	rawSerialize, err := t.Serialize()
	if err != nil {
		return nil, err
	}
	withness := make([][]byte, len(t.Witnesses))
	for i := 0; i < len(t.Witnesses); i++ {
		od, err := t.Witnesses[i].Serialize()
		if err != nil {
			return nil, err
		}
	...
	withnessBytes := SerializeDynVec(withness)
	fields := [][]byte{rawSerialize, withnessBytes}
	return SerializeTable(fields), nil
}
