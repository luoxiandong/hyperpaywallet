...
func DecodeSegWitAddress(address string) (byte, []byte, error) {
	...
	regrouped, err := ConvertBits(data[:], 5, 8, false)
	if err != nil {
		return 0, nil, err
	}
	len := len(regrouped)
	hash := regrouped[len-20:]
...
func EncodeSegWitAddress(hrp string, hash []byte) (string, error) {
	...
	data_part, err := ConvertBits(payload, 8, 5, true)
	if err != nil {
		return "", err
	}
	...
	bech, err := Bech32Encode(hrp, data_part)
	if err != nil {
		return "", err
	}
