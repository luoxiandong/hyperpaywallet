...
func Test_case1(t *testing.T) {
	...
	if err != nil {
		...
		fmt.Println("pubkey len :", len(sigPub.Pubkey))
		fmt.Println("pubkey :", hex.EncodeToString(sigPub.Pubkey))
	}
	...
	if err != nil {
		t.Error("插入失败")
		fmt.Println("signedTrans err = ", err.Error())
	} else {
		fmt.Println("合并后的交易单:")
...
func Test_case2(t *testing.T) {
	// item1 := hpywallet.BTMOutPutItem{
	// 	SourceID:       "e12a002a479694f6d2c1234219b0921815aa9671dc768e1d7bea336f31b2e1a2",
	// 	Amount:         uint64(2000000000),
	// 	AssetID:        "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
	// 	SourcePosition: uint64(1),
	// 	ControlProgram: "0014ffae7d0eb1f969ad603d1d3c77edb74022c64d18",
	// }

	// 第一个UTXO
	sourceID1 := "e12a002a479694f6d2c1234219b0921815aa9671dc768e1d7bea336f31b2e1a2"
	sourcePos1 := uint64(1)
	assetID1 := "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
	amount1 := uint64(2000000000)
	controlProgram1 := "0014ffae7d0eb1f969ad603d1d3c77edb74022c64d18"

	in1 := Vin{sourceID1, sourcePos1, assetID1, amount1, controlProgram1}

	// 第一个输出
	address1 := "bm1q2an2jkvrccwedu9cpnwehc7lplwu70pgemwx3k"
	sendAmount1 := uint64(2000000)

	out1 := Vout{address1, sendAmount1}

	// 第二个输出
	address2 := "bm1ql7h86r43l9566cpar5780mdhgq3vvngcxmgf6u"
	sendAmount2 := uint64(1996000000)
	...
	// 构建空交易单
	emptyTrans, err := CreateEmptyRawTransaction([]Vin{in1}, []Vout{out1, out2}, timeRange, false)
	...
	fmt.Println("priKey len = ", len(hex.EncodeToString(prikey1)), "privateKey = ", hex.EncodeToString(prikey1))
	// 对交易单的第一个输入的哈希进行签名
