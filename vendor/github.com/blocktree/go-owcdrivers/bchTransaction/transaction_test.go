...
func Test_case1(t *testing.T) {
	// 前置交易ID和UTXO对应的索引
	in := Vin{"7d06a6de272252d976f9e3c4ec2cc17af5ba768484b9053f2743ebaa729a953f", uint32(0)}
	// 目标地址和发送金额

	out := Vout{"1JjAnPSJ5JzHCv1XPV9mWbwApLmGbmkM3f", uint64(200000)}
	changeOut := Vout{"1N6r79HnNcsJiue4uMbqyNDTHuJQdqN98n", uint64(5000000 - 200000 - 200000)}
	...
	addressPrefix := BCHMainnetAddressPrefix

	///////构建空交易单
	emptyTrans, err := CreateEmptyRawTransaction([]Vin{in}, []Vout{out, changeOut}, lockTime, replaceable, addressPrefix)
	...
	//获取地址用于区分签名哈希
	// 填充TxUnlock结构体
	inLock := "76a914e775b67b59040436c4350278f38c3f4e9d283a6688ac"
	//	inLock := "76a914d46043209073ad39879356295562d952cd9dae3a88ac"

	//指向此类型地址的UTXO，获取签名哈希需要锁定脚本,赎回脚本应设置为 ""
	unlockData := TxUnlock{inLock, "", 5000000, SigHashALLFork}
	...
	//inPrikey := []byte{0x80, 0xbc, 0x39, 0x8d, 0x7c, 0x4a, 0x67, 0x4d, 0xaa, 0x97, 0x75, 0x66, 0xc2, 0xe6, 0xcd, 0x50, 0x40, 0x52, 0x00, 0x27, 0xe5, 0x7f, 0xe8, 0x06, 0xdf, 0xaa, 0x86, 0x8d, 0xf4, 0xcc, 0x43, 0xab}
	inPrikey, _ := hex.DecodeString("8a2fdd64eb97cdcedbd0612a11972b8c9cec2ac2b58e4223732575e198585d80")
	//签名
