...
func newEmptyTransaction(vins []Vin, vouts []Vout, lockTime uint32, replaceable bool, addressPrefix AddressPrefix, blockHash string, blockHeight uint64) (*Transaction, error) {
	...
	txOut, err := newTxOutForEmptyTrans(vouts, addressPrefix, blockHash, blockHeight)
	if err != nil {
		return nil, err
	}
...
func DecodeRawTransaction(txBytes []byte, SegwitON bool) (*Transaction, error) {
	...
	index += 4
	fmt.Println(littleEndianBytesToUint32(rawTx.Version))
