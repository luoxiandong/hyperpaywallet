...
func NewTxStruct(from, pubkey string, sequence uint32, to string, amount, fee uint64, signature string, lastLedgerSequence uint32, memoType, memoData, memoFormat string) (*TxStruct, error) {
	...
	tag, err := strconv.ParseInt(memoData, 10, 32)
	if err != nil {
		txStruct.DestinationTag = []byte{}
	} else {
		desTag := uint32(tag)
		tagByte := getDestinationTagBytes(desTag)
		txStruct.DestinationTag = tagByte
	}
	return &txStruct, nil
}
...
func (tx TxStruct) ToEmptyRawWiths() string {
	...
	if len(tx.DestinationTag[:]) == 0 && len(tx.Memos) > 0 {
